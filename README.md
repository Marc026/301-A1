...
> **HINT:**
> The code for `SaveImage` is very similar to the code for `LoadImage`.  The 
> only difference is that `SaveImage` will write to an `ofstream` object 
> instead of reading to a `ifstream` object.  You will need to use an 
> ofstream instead of an ifstream and switch `>>` to  `<<`.  You will also 
> need to add white space between values.
> 
> ```
> imageInputFile >> image[x][y][RED];  // reading from a ifstream
> 
> imageOutputFile << image[x][y][RED] << " ";  // writing to a ofstream 
> ```
...
This private function will not be graded.  However, it is a good idea to 
implement since there are many functions in the program the check the bounds 
of an (x, y) coordinate.  This function 
should return true if a given (x, y) is within the bounds of the image and 
false if the (x, y) is outside the bounds of the image.  The function should 
NOT perform any throw.
